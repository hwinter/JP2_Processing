...
def AIA_AddInfographic(BASE, OVERLAY, OUTNAME): #BASE: Output of AIA_GenerateBackground(), Overlay: the graphical overlay image EG: QFA_Wall_Base2x2.mp4, OVERLAY_2x2.png
	...
	while(cap.isOpened()):
	    ...
	    if ret == True:
	        ...
	        cv2.imwrite("QFA_out" + str(i) + ".png", outImage)
	        i = i + 1
	...
	subprocess.call('ffmpeg -r 24 -i QFA_out%01d.png -vcodec libx264 -b:v 4M -pix_fmt yuv420p -y ' + str(OUTNAME), shell=True)

	for f in glob.glob("QFA_out*.png"):
	    os.remove(f)
...
if __name__ == '__main__':
	try:	
		...
		for wlen in target_wavelengths:
			...
			sorted_list = Build_Index(str("live/" + wlen))
		...
		final_outname = str(year) + "_" + str(month) + "_" + str(day) + "_QFA_VideoWall_Concatenated.mp4"
		...
		# for f in glob.glob("QFA_SegmentOverlay_*.mp4"):
		# 	    os.remove(f)
